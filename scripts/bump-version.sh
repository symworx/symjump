...
#
# Bump the shared symjump workspace version in lockstep.
#
# Source of truth: root Cargo.toml  [workspace.package] version
# Member crates use version.workspace = true and depend on internal
# crates via `{ workspace = true }`.
#
# Updates (only these sites):
#   1. [workspace.package] version
#   2. Internal symjump-* pins under [workspace.dependencies]
#      (path + version; Cargo forbids version.workspace = true there)
#   3. Cargo.lock entries for workspace members (sjmp, symjump-config,
#      symjump-core)
#   4. CHANGELOG.md [X.Y.Z] tag link and [Unreleased] compare URL
...
#      Publish crates.io by hand until a tag workflow exists:
#      cargo publish -p symjump-config && cargo publish -p symjump-core
...
LOCK_PACKAGES=(sjmp symjump-config symjump-core)
REPO_URL="https://github.com/symworx/symjump"
...
read_cargo_internal_versions() {
  awk '
    ...
    in_deps && $0 ~ /^\[/ { in_deps=0 }
    in_deps && $0 ~ /^symjump-/ && match($0, /^symjump-[a-z0-9-]+/) {
...
bump_changelog_links() {
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    ...
    echo "CHANGELOG.md: would point [Unreleased] compare at v${new}...HEAD"
    return 0
  fi
  local tmp
  tmp="$(mktemp)"
  awk -v new="$new" -v repo="$REPO_URL" '
    BEGIN { inserted = 0 }
    /^\[Unreleased\]:/ {
...
bump_cargo_lock() {
  ...
  tmp="$(mktemp)"
  awk -v new="$new" '
    $0 ~ /^name = "sjmp"$/ || $0 ~ /^name = "symjump-config"$/ || $0 ~ /^name = "symjump-core"$/ {
      hit=1
      print
      next
    }
...
EOF
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    echo "--- dry-run: would insert CHANGELOG stub for [${new}] after Unreleased ---"
    echo "$stub"
    return 0
  fi
  local tmp
...
if [[ "$DIRECTION" == "downgrade" ]]; then
  ...
  echo "  3. Commit, PR → worx"
  echo "  4. After merge:  git tag -a v${NEW} -m v${NEW} && git push origin v${NEW}"
  echo "     Then publish by hand:  cargo publish -p symjump-config"
  echo "                            cargo publish -p symjump-core"
  echo "                            cargo publish -p sjmp"
fi
echo
